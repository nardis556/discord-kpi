...
async def alert_user(user_id):
    global channel
    try:
        await channel.send(f"<@{user_id}> The next competition is about to begin! Head over to #wl-start-here and add a NEW WALLET to participate")
...
# @retry(stop=stop_after_attempt(10), wait=wait_fixed(2), sleep=1) 
# async def manage_roles(user_id):
